...
PREFERRED_MODELS = [
    ...
    "models/gemini-1.5-pro",   # limited free usage
]
...
def diagnose():
    ...
    output_box.config(state=tk.NORMAL)
    output_box.delete("1.0", tk.END)
    header = f"Using model: {CHOSEN_MODEL}\n\n"
    output_box.insert(tk.END, header + result)
    output_box.config(state=tk.DISABLED)
...
# Diagnose button
btn = tk.Button(root, text="Diagnose", command=diagnose, bg="#0f62fe", fg="white", font=("Arial", 12), padx=10, pady=5)
btn.pack(pady=10)
